...
class Window(QMainWindow):
    ...
    def save_file(self, tabs=None, index=None):
        ...
        if path:
            if isinstance(widget, QtWidgets.QTextEdit):
                ret = self._save_to_path(widget.toPlainText(), path)
                widget.document().setModified(False)
                return ret
            elif isinstance(widget, QtWidgets.QScrollArea):
                editors = widget.findChildren(QtWidgets.QTextEdit)
                saved = self._save_to_path(editors[0].toPlainText(), path)
                editors[0].document().setModified(False)
                for ed in editors[1:]:
                    if not self._append_to_path("\n#---\n"+ed.toPlainText(), path):
                        saved = False
                    else:
    ...
    def on_tab_close(self, tabs, index):
        ...
        try:
            if widget.parent().parent().tabText(index)[0] == '*':
